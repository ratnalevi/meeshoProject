...
class HTTPResponse {
	...
	public function getDetailedMessage($statusCode){
		...
		return ($statusMsg[$statusCode]) ? $statusMsg[$statusCode] : $statusMsg[500];
	}

	public function getHTMLResponse($statusCode){
		$msg = $this->getDetailedMessage($statusCode);
		$reason = $this->getHttpStatusMessage($statusCode);
		$html = '<!DOCTYPE HTML PUBLIC "-//IETF//DTD HTML 2.0//EN">n<html>';
  		$html .= "<head>n<title>{$statusCode} {$reason}</title>n<b>{$reason}</b>n<p>{$msg}<br />n</p>n";
  		return $html;
	}
}
?>
